    ...
    <div className="bg-background text-foreground overflow-x-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-20 items-center">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="space-y-8">
            ...
            <p className="text-gray-400 text-xl max-w-md leading-relaxed font-medium">
              We don’t just ship cargo; we provide the <span className="text-white">infrastructure for global commerce.</span> Experience unmatched precision and real-time intelligence.
            </p>
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <Link href="/services" className="glass-card px-10 py-5 font-black text-xs tracking-widest rounded-full hover:bg-foreground/5 transition-all text-foreground uppercase border-white/10">
                Explore Fleet
              </Link>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-12 rounded-[3.5rem] shadow-3xl backdrop-blur-3xl border-white/5 relative overflow-hidden group">
            ...
            <form onSubmit={handleTrack} className="relative">
              <input type="text" value={trackId} onChange={(e) => setTrackId(e.target.value)} placeholder="e.g. TRK-8829-XJ" className="w-full bg-foreground/5 border border-glass-border rounded-3xl p-6 focus:outline-none focus:border-primary text-foreground placeholder:text-gray-600 font-bold pr-24 text-lg" />
      ...
      <section className="py-10 border-y border-white/5 bg-foreground/[0.02]">
         <div className="max-w-7xl mx-auto px-6">
            <p className="text-center text-[10px] font-bold text-gray-500 uppercase tracking-[0.4em] mb-8">Trusted by Global Industry Leaders</p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-32">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <motion.div key={i} variants={fadeInUp} className="p-12 glass-card rounded-[3rem] hover:bg-primary transition-all group cursor-pointer border-white/5">
      ...
      {/* 4. THE ADVANTAGE SECTION (ENTICING TEXT) */}
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div className="space-y-8">
                ...
                <div className="space-y-6">
                    <AdvantageItem icon={<Zap />} title="AI-Route Optimization" desc="Our neural networks calculate the fastest path, bypassing port congestion and delays automatically." />
                    <AdvantageItem icon={<Shield />} title="Military-Grade Security" desc="Every shipment is monitored via encrypted satellite links with 100% insurance coverage." />
                    <AdvantageItem icon={<BarChart3 />} title="Live Analytics" desc="Get deep insights into your supply chain health with our real-time reporting dashboard." />
                </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 text-center">
         <div className="glass-card p-16 rounded-[4rem] border-white/5">
            <Headphones size={48} className="mx-auto text-primary mb-6 animate-pulse" />
            <h2 className="text-4xl font-black uppercase tracking-tighter mb-4 text-white">Need Personal Assistance?</h2>
            <p className="text-gray-400 max-w-md mx-auto mb-10 font-medium text-lg">Our global strategy team is available 24/7 to solve your logistics complexities.</p>
